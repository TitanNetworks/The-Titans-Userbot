...
# Making it easy....
# thanks to @ranger_op (mujhe pata bhi nahi tha ki mera bnaya hua mujhe vapas kang krna pdega )
# codes by @mrconfused
# catuserbot
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            Titanresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            Titanfile = await event.client.download_media(Titanresponse, "./temp")
            return await unzip(Titanfile)
