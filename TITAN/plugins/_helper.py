...
async def yardim(event):
    ...
    if tgbotusername is not None or Titan_input == "text":
        results = await event.client.inline_query(tgbotusername, "@THETITANS_USERBOT")
...
async def info(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} commands found in {plugincount} sudo plugins of ⚜️ Tʜᴇ ᴛɪᴛᴀɴꜱ ᴜꜱᴇʀʙᴏᴛ ⚜️\n\n"
        )
        Titancount = 0
        plugincount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += "\n"
        if len(string) > 4095:
            data = string.format(count=Titancount, plugincount=plugincount)
            key = (
                requests.post(
            ...
            reply_text = f"All commands of ⚜️ Tʜᴇ ᴛɪᴛᴀɴꜱ ᴜꜱᴇʀʙᴏᴛ ⚜️ are [here]({url})"
            await event.reply(reply_text, link_preview=False)
            return
        await event.reply(
            string.format(count=Titancount, plugincount=plugincount), link_preview=False
        )
        return
    if input_str:
        if input_str in SUDO_LIST:
            ...
            for i in SUDO_LIST[input_str]:
                ...
                Titancount += 1
            await event.reply(
                string.format(count=Titancount, input_str=input_str), parse_mode="HTML"
            )
        else:
       ...
       Titancount = 0
        for i in sorted(SUDO_LIST):
            ...
            string += " "
            Titancount += 1
        await event.reply(string.format(count=Titancount), parse_mode="HTML")
