            ...
            if not show.pattern_match.group(1):
                async for user in show.client.iter_participants(show.chat_id):
                    if not user.deleted and user.id != bot.uid:
                        mentions += f"\n{user.id},â ï¸Porn / Porn Group Member//AntiPornFed #Massbanðð"
    ...
    if not show.text[0].isalpha() and show.text[0] not in ("/"):
        ...
        try:
            if not show.pattern_match.group(1):
                async for user in show.client.iter_participants(show.chat_id):
                    if not user.deleted and user.id != bot.uid:
                        mentions += f"\n{user.id},â ï¸Suspicious/Btc Scammer/Fraudulent activities #Massbanð"
                    elif user.id != bot.uid:
                        mentions += f"\n{user.id},â ï¸Suspicious/Btc Scammer/Fraudulent activities #Massbanð"
            else:
                searchq = show.pattern_match.group(1)
...
async def _(event):
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(3)
    await event.edit(f"SuperUnFBan Completed by titans master . Affected {len(fedList)} feds.")
